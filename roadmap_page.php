...
foreach ($t_project_ids as $t_project_id) {
    ...
    foreach ($t_version_rows as $t_version_row) {
        ...
        while (0 < count($t_issue_ids)) {
            ...
            if ($eisFlag == 1) {
                $eisBug = bug_get($t_issue_id);
                $eisBugs[] = array("bug" => $eisBug, "parent" => $t_issue_parent);
            }
...
function eisShowPert($bugs = array()) {
    print '<script src="/js/anychart-custom-build.min.js" type="text/javascript"></script>';
    print '<div id="container" style="width: 500px; height: 400px;"></div>';
    ...
    foreach ($bugs as $eisBug) {
        ...
        if (!isset($idxBug[$bug->id])) {
            $idxBug[$bug->id] = $position++;
        }
        if (!isset($links[$bug->id])) {
            $timeAll += $eta;
            if ($bug->status >= 80) {
                $timeDone += $eta;
            }
        ...
        if ($eisBug["parent"] > 0) {
            if (!isset($idxBug[$eisBug["parent"]])) {
                $idxBug[$eisBug["parent"]] = $position++;
            }
            $links[$eisBug["parent"]][] = $idxBug[$bug->id];
            if (!isset($dataRecord[$eisBug["parent"]])) {
                $bugP = bug_get($eisBug["parent"]);
                $dataRecord[$bugP->id] = "id:\"" . $idxBug[$bugP->id] . "\", duration: " . 1 . ", name: \"" . $bugP->id . "\"";
            }
        }
        if ($bug->source_bug_id > 0) {
            if (!isset($idxBug[$bug->source_bug_id])) {
                $idxBug[$bug->source_bug_id] = $position++;
            }
            $links[$bug->source_bug_id][] = $idxBug[$bug->id];
            if (!isset($dataRecord[$bug->source_bug_id])) {
                $bugP = bug_get($bug->source_bug_id);
                $dataRecord[$bugP->id] = "id:\"" . $idxBug[$bugP->id] . "\", duration: " . 1 . ", name: \"" . $bugP->id . "\"";
            }
        }
        $dataRecord[$bug->id] = "id:\"" . $idxBug[$bug->id] . "\", duration: " . $eta . ", name: \"" . $bug->id . "\", summary:\"" . htmlspecialchars($bug->summary) . "\", statusId: ".$bug->status.", status: \"" . MantisEnum::getLabel(lang_get('status_enum_string'), $bug->status) . "\"";
    }
    foreach ($links as $sId => $tIds) {
        $text = implode("\",\"", $tIds);
        if ($text != "") {
            $dataRecord[$sId] .= ", dependsOn: [\"" . implode("\",\"", $tIds) . "\"]";
        }
    }
    print "{" . implode("},\n{", $dataRecord) . "}";
    print '];
                ...
                chart.tooltip(true);
                chart.tasks().tooltip().useHtml(true);
                var tooltip = chart.tasks().tooltip();
                tooltip.format("Задача: <b><a href=\"view.php?id={%name}\">{%name}</a></b>\nПродолжительность: <b>{%duration}</b><br />{%summary}<br />Статус: {%status}");
                var background = chart.tasks().tooltip().background();
...
chart.milestones().labels().format(function() {
    if (this.creator) {
        ...
        for (i = 0; i < this.predecessors.length; i++){
            if(this.predecessors[i].get("statusId") >= 80){
                status ++;
            }
        }
        if(this.predecessors.length == status){
            result = " + ";
        }else{
            result = " - ";
        }
        //result += this.creator.get("name");
        return result;
...
// set the chart title to show the duration
                chart.title("Продолжительность " + duration + " д.");
...
function eisShowPertOld($bugs = array()) {
    $data = 'digraph untitl {
	node [ fontname="Helvetica-Bold", fontsize="8", shape="circle", width="0.3", style ="setlinewidth(2),filled" color="black", fillcolor = "pink"];
	edge [ fontname="Helvetica-Bold", fontsize="10", style="bold" ];
    ...
    foreach ($bugs as $eisBug) {
        $bug = $eisBug["bug"];
        $data .= $bug->id . '[ fillcolor="' . get_status_color($bug->status) . '"];
            ';
        switch ($bug->eta) {
            case 20: {
                    $eta = "1";
        ...
        if ($eisBug["parent"] > 0) {
            $links[$bug->id][$eisBug["parent"]] = $eta;
        }
        if ($bug->source_bug_id > 0) {
            $links[$bug->id][$bug->source_bug_id] = $eta;
        } else {
//            $link = $bug->id . " -> " . $bug->id . " [ label=\"" . $eta . "\"];\n";
        }
    }
    foreach ($links as $sId => $tIds) {
        foreach ($tIds as $tId => $v) {
            $data .= $sId . " -> " . $tId . " [ label=\"" . $v . "\"];\n";
        }
    }
    $data .= '}';
    $filename = tempnam("/tmp", "dot");
    $filenamePng = $filename . ".png";
    file_put_contents($filename, $data);
    system("dot -Tpng " . $filename . " > " . $filenamePng);
    $content = file_get_contents($filenamePng);
    print "<img src='data:image/png;base64," . base64_encode($content) . "' /><br />";
    unlink($filenamePng);
    unlink($filename);
    if ($timeAll > 0 && $timeDone / $timeAll > 0.6) {
        print "<span style='color:green;'>";
    } else {
        print "<span style='color:red;'>";
    }
    print "Прогресс: " . $timeDone . " из " . $timeAll . " дней";
    print "</span><br />";
}
